...
# ---------------- TCP JPEG Frame Client ----------------
class TcpJpegFrameClient:
    """
    Raspi의 pinky_cam_server.py가 보내는 length-prefix JPEG 프레임을 수신.
    - 별도 스레드에서 지속 수신
    - latest RGB 프레임을 저장
    """

    def __init__(self, host: str, port: int, reconnect_sec: float = 1.0, sock_timeout: float = 3.0):
        self.host = str(host)
        self.port = int(port)
    ...
    def get_latest_age_sec(self) -> float:
        with self._lock:
            if self._latest_t <= 0.0:
                return 1e9
            return time.time() - self._latest_t
        ...
        while not self._stop.is_set():
            ...
            try:
                ...
                while not self._stop.is_set():
                    ...
                    if hdr is None:
                        break
                    (payload_len,) = struct.unpack("!I", hdr)
...
class PrecisionController(Node):
    def __init__(self):
        ...
        self.declare_parameter("action_name", "actions/follow_aruco")

        # ---- TCP camera input ----
        self.declare_parameter("tcp_host", "127.0.0.1")   # 보통 raspi 로컬이면 127.0.0.1 (서버도 raspi에서 돌리니까)
        self.declare_parameter("tcp_port", 9001)
        self.declare_parameter("tcp_frame_max_age_sec", 0.6)
        ...
        self.declare_parameter("angle_tol_rad", 0.08)
            # angle_tol_rad	  각도(°)	    체감
            # 0.02	          ~1.1°	       🔴 매우 빡빡 → 떨림 거의 확실
            # 0.03	          ~1.7°	       ⚠️ 환경 좋으면 OK, 노이즈 있으면 불안
            # 0.05	          ~2.9°	       ✅ 추천 기본값
            # 0.08	          ~4.6°	       🟡 안정적, 약간 느슨
            # 0.10	          ~5.7°	       🟢 흔들림 거의 없음, 정밀도↓

        # lateral option (same as reference; set tol=0 to disable)
        self.declare_parameter("lateral_tol_m", 0.03)
            # 0이면 항상 lateral 플랜이 비활성화
            # 활성화시:
            #     90도 회전
            #     앞으로 직진(x_move 만큼)
            #     다시 원래 방향으로 회전
            # 이라서 결과적으로 옆으로 평행이동(side-step) 효과를 내는 방식
        ...
        self.tcp_frame_max_age_sec = float(self.get_parameter("tcp_frame_max_age_sec").value)
        ...
        # ---- TCP camera client ----
        self.cam = TcpJpegFrameClient(self.tcp_host, self.tcp_port, reconnect_sec=1.0, sock_timeout=3.0)
        self.cam.start()
        ...
        self.get_logger().info(
            ...
            f"timeout={self.timeout_sec:.1f}s), tcp={self.tcp_host}:{self.tcp_port})"
        )
    ...
    # ---------------- helpers ----------------
    # LOCK(중앙값 확정)된 값들을 가지고 “이제 로봇이 어떤 순서로 움직일지(plan 리스트)”를 만드는 함수
    # LOCK 단계에서 median으로 확정된 값들:
    #     locked_err_z : dz - target_z (앞/뒤로 얼마나 더 가야 목표거리인지)
    #     locked_dx_m : dx (좌/우로 얼마나 치우쳤는지)
    #     locked_yaw_z : 마커 z축 기반 yaw(방향) 정보 (정면 정렬에 사용)
    #     그리고 현재 odom의 current_yaw (로봇 현재 요)
    def _build_plan_from_lock(
        ...
        *,
        ...
        yaw_z_target = normalize_angle(locked_yaw_z + math.pi)
            # locked_yaw_z는 “카메라 기준으로 마커 z축이 바라보는 방향” 같은 값인데,
            # 보통 마커를 정면으로 바라보려면 z축 방향에서 180도(π) 반대 방향으로 서야 해.
            # 그래서 + pi 해 주고 normalize_angle로 -π~π 범위로 정리.

        # yaw step에서 쓸 “절대 목표 yaw” 만들기
        yaw_align_goal = normalize_angle(current_yaw + yaw_z_target)
        yaw_back = normalize_angle(current_yaw)
            # yaw_back은 side-step(옆으로 비켜가기) 끝난 뒤 “원래 바라보던 방향으로 돌아오기” 목표.

        # 이동량(거리) 계산: z와 x
        z_move = float(locked_err_z)  # err_z = (dz - target_z)
            # dz - target_z 이므로
            # dz가 목표보다 크면(+) → 앞으로 가야 함(가까워져야 함)
            # dz가 목표보다 작으면(-) → 뒤로 가야 함(멀어져야 함)
        corrected_dx_m = float(locked_dx_m)
        x_move = abs(corrected_dx_m)
            # lateral(옆으로 비켜가기) 플랜을 만들 때, 옆으로 얼마나 이동할지 거리로 씀
            # dx의 부호는 방향 결정을 위해 따로 보관(아래에서 side_sign)
        ...
        if await_relock:
            ...
            return plan, False
                # 만약 이전 플랜에서 lateral side-step을 수행했다면,
                # 그 후엔 relock을 하고 다시 LOCK을 잡을 거야.
                # 그 “두 번째 LOCK” 이후에는 또 lateral을 반복하면 안 되고,
                # 이제는 z 이동 + yaw 정렬만 하도록 강제하는 것.

        # lateral correction이 켜져 있고, dx가 tol보다 크면: side-step 플랜 생성
        # lateral 기능이 활성화(>0) 되어 있고
        # 옆으로 치우친 정도가 허용치보다 크면
        # “옆으로 비켜가기”를 실행
    ...
    async def _execute_cb(self, goal_handle):
        ...
        try:
            while rclpy.ok():
                ...
                # TCP frame not ready / stale
                frame_rgb = self.cam.get_latest_rgb()
                if frame_rgb is None or self.cam.get_latest_age_sec() > self.tcp_frame_max_age_sec:
                    stop_robot()
                    time.sleep(0.01)
                    continue
                ...
                if not has_lock:
                    ...
                    if det is None:
                        ...
                        if len(err_z_samples) > 0:
                            # 이미 샘플을 몇 개 모으는 중이었는데 마커가 끊기면
                            # “회전 계속”하지 말고 일단 정지(stabilize)
                            # 다음 루프에서 다시 보이면 샘플링 재개
...
### 실행 예시:
# 1) Raspi에서 카메라 서버 1개만 실행(카메라 소유권)
# ros2 run sensors image_socket

# ros2 run actions follow_aruco_action 

### 시스템1에서 해야할 일을 간단 버전으로 테스트 할 수 있는 실행문
